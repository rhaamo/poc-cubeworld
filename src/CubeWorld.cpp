...
void CubeWorld::createScene(void) {
    ...
    scnMgr->setSkyDome(true, "SkyDome1", 2, 8, 100);
    ...
    scnMgr->setAmbientLight(Ogre::ColourValue(0.02, 0.02, 0.02));

}

Ogre::ManualObject *CubeWorld::createCubeMesh(Ogre::String name, Ogre::String matName) {
    ManualObject *cube = new Ogre::ManualObject(name);
    cube->begin(matName);
    ...
    for (int z = 0; z < WORLD_SIZE; ++z) {
        for (int y = 0; y < WORLD_SIZE; ++y) {
            for (int x = 0; x < WORLD_SIZE; ++x) {
                if (sqrt((float) (x - WORLD_SIZE / 2) * (x - WORLD_SIZE / 2) +
                         (y - WORLD_SIZE / 2) * (y - WORLD_SIZE / 2) + (z - WORLD_SIZE / 2) * (z - WORLD_SIZE / 2)) <
                    WORLD_SIZE / 2)
                    GetBlock(x, y, z) = 1;
            }
        }
...
void CubeWorld::displaySimpleWorld(void) {
    ...
    Ogre::StaticGeometry *pGeom = new Ogre::StaticGeometry(scnMgr, "Boxes");

    Ogre::Entity *pEnt = scnMgr->createEntity("TestBox2");
    ...
    for (int layer = 0; layer < NumLayerMaps; ++layer) {
        ...
        for (int z = 0; z < WORLD_SIZE; ++z) {
            for (int x = 0; x < WORLD_SIZE; ++x) {
                ...
                int Height = (Map.GetValue(x, z) + 1) / 2.0f * (g_Layers[layer].MaxLevel - g_Layers[layer].MinLevel) +
                             g_Layers[layer].MinLevel;
...
void CubeWorld::createChunkWFaces(const int StartX, const int StartY, const int StartZ) {
        ...
        for (int y = StartY; y < CHUNK_SIZE + StartY; ++y) {
            for (int x = StartX; x < CHUNK_SIZE + StartX; ++x) {
                ...
                if (Block1 == 0) {
                    ...
                    MeshChunk->textureCoord(0, V2);
                    MeshChunk->position(x, y + 1, z + 1);
                    MeshChunk->normal(-1, 0, 0);
                    MeshChunk->textureCoord(1, V2);
                    MeshChunk->position(x, y + 1, z);
                    MeshChunk->normal(-1, 0, 0);
                    MeshChunk->textureCoord(1, V1);
                    MeshChunk->position(x, y, z);
                    MeshChunk->normal(-1, 0, 0);
                    MeshChunk->textureCoord(0, V1);
    ...
    MeshChunk->end();
    scnMgr->getRootSceneNode()->createChildSceneNode()->attachObject(MeshChunk);
...
void CubeWorld::createChunkWater(const int StartX, const int StartY, const int StartZ) {
    ...
    ManualObject *MeshChunk = new ManualObject("MeshWaterChunk" + StringConverter::toString(m_ChunkID));
...
void CubeWorld::createWorldChunks(void) {
    ...
    MaterialPtr mat = MaterialManager::getSingleton().create("BoxColor", "General", true);
    Technique *tech = mat->getTechnique(0);
    Pass *pass = tech->getPass(0);
    TextureUnitState *tex = pass->createTextureUnitState();
    tex->setColourOperationEx(LBX_MODULATE, LBS_MANUAL, LBS_CURRENT, ColourValue(0, 0.5, 0));

    ManualObject *MeshChunk = new ManualObject("MeshMatChunk");
    MeshChunk->begin("TerrainImage");
...
void CubeWorld::createTexture(const TCHAR *pName, const TCHAR *pImageFilename) {
    MaterialPtr mat = MaterialManager::getSingleton().create(pName, "General", true);
    Technique *tech = mat->getTechnique(0);
    Pass *pass = tech->getPass(0);
    TextureUnitState *tex = pass->createTextureUnitState();
    ...
    tex->setTextureFiltering(FO_POINT, FO_POINT, FO_POINT);

    pass->setVertexColourTracking(TVC_DIFFUSE);
    tex->setColourOperationEx(LBX_MODULATE, LBS_DIFFUSE, LBS_TEXTURE);
...
void CubeWorld::createWaterTexture(const TCHAR *pName) {
    ...
    tex->setColourOperationEx(LBX_SOURCE1, LBS_MANUAL, LBS_CURRENT, ColourValue(0, 0, 1));
    tex->setAlphaOperation(LBX_SOURCE1, LBS_MANUAL, LBS_CURRENT, 0.5);
}
...
void CubeWorld::createSkyTexture(const TCHAR *pName) {
    ...
    tex->setColourOperationEx(LBX_MODULATE, LBS_MANUAL, LBS_CURRENT, ColourValue(1, 1, 1));
...
void CubeWorld::updateSkyTextureLight(void) {
    ...
    tex->setColourOperationEx(LBX_MODULATE, LBS_MANUAL, LBS_CURRENT, m_LightColor);
}
